...
/// <summary>check_overshoot(*y + a * *x)</summary>
...
/// <summary>check_overshoot(*xx)</summary>
...
{
	double labound;	// amount by which to scale up vector, for reduce_inner
	double temp;
...
// ----------------------------------------------------------------
static void connecting(simplex *s) {
...
static site get_next_site(void)
{
	...
	// if there is a list of integer points
	if (ptrToIntsToIndex)
		for (i=0; i<pdim; i++)
			p[i] = *ptrToIntsToIndex++;
	// else convert the floating points to integers
	else
		for (i = 0; i < pdim; i++)
		{
...
static void buildhull(simplex* root)
{
	while (cdim < rdim)
	{
		...
		if (out_of_flat(root,p))
			...
			connecting(make_facets(search(root)));
	}
